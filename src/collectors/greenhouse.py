"""
Greenhouse ATS collector — thin wrapper around src.collectors.ats.
Supports standalone `--test` execution via `python -m src.collectors.greenhouse --test`.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Greenhouse Jobs Collector")
    ...
    try:
        jobs, failures = collect(dedup, platform="greenhouse")
        for j in jobs:
            if j.get("company"):
                scanned_companies.add(j["company"])
        errors = len(failures)
    except Exception as exc:
        logger.error(f"Greenhouse test failed: {exc}")
        jobs = []
        errors += 1
    ...
    print("\n--- Greenhouse Collector Smoke Test Report ---")
