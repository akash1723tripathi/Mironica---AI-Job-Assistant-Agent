"""
Lever ATS collector — thin wrapper around src.collectors.ats.
Supports standalone `--test` execution via `python -m src.collectors.lever --test`.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Lever Jobs Collector")
    ...
    try:
        jobs, failures = collect(dedup, platform="lever")
        for j in jobs:
            if j.get("company"):
                scanned_companies.add(j["company"])
        errors = len(failures)
    except Exception as exc:
        logger.error(f"Lever test failed: {exc}")
        jobs = []
        errors += 1
    ...
    print("\n--- Lever Collector Smoke Test Report ---")
